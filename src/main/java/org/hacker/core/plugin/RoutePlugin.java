...
/**
 * <p>
 * 自定义的默认路由基类，该类通过反射将Controller（in general the pageName is
 * "com.zjhcsoft.mvc.controller"）
 * 中的所有继承了Controller的java类定义默认的路由规则('包名+类名(除去Controller)')，
 * 而不必每个路由都需要我们自己在JFinal中去定义，默认规则是：me.add("XXX", XXX.class);
 * </p>
 * 
 * <p>
 * 如:com.zjhcsoft.mvc.controller.admin.UserController
 * <p>
 * 则路由规则为: '/admin/user' View路径和路由路径相同 全部为小写
 * <p>
 * 注意:com.zjhcsoft.mvc.controller.admin.Usercontroller
 * <P>
 * XXXController是路由类的规范，则否会抛出异常
 * 
 * @version 2.0
    ...
    try {
      ...
      for (File f : files) {
        ...
        if (mvc_controller.isAssignableFrom(c) && c.getName().contains(prefix)) {
          ...
          oldClassName = oldClassName.substring(oldClassName.indexOf(prefix) == 0 ? prefix.length() + 1
              : oldClassName.indexOf(prefix) + prefix.length() + 1);
