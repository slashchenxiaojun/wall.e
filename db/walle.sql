...
CREATE TABLE `w_interface` (
  ...
  `data` varchar(5000) DEFAULT NULL COMMENT '模拟数据',
  `result_data` varchar(5000) DEFAULT NULL,
...
CREATE TABLE `w_parameter` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `name` varchar(45) NOT NULL,
  `type` enum('integer','enum','boolean','number','date','string','byte','object','array') NOT NULL DEFAULT 'string' COMMENT '对应java中的关系\nbool: boolen\nnumber: int, long, flow, double, BigDecimal\ndate: java.util.date\nstring: string\nbyte: object\njsonobj: jsonObject\njsonarray: jsonArray\n''bool'',''numbe'',''date'',''string'',''byte''',
  `min` varchar(45) DEFAULT '0',
  `max` varchar(45) DEFAULT NULL,
  `format` varchar(500) DEFAULT NULL COMMENT 'format是存在于当type=''date''\n相当于时间格式yyyy-MM-dd hh:mm:ss\n或者是其他的格式，可用于扩展',
  ...
  `array_type` varchar(255) DEFAULT 'string' COMMENT '当type=jsonarray时，泛型的类型',
